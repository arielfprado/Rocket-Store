...
export const ShoppingCartContext = createContext({} as IShoppingCartContextData)

export function ShoppingCartProvider(
  props: IShoppingCartProviderProps
): JSX.Element {
  const [shoppingCart, setShoppingCart] = useState<ISCProductProps[]>(() => {
    const storagedProducts = localStorage.getItem(localstorageNames.cart)
  ...
  function handleAddProductOnShoppingCart(product: ISCProductProps) {
    if (isProductInShoppingCart(product.id)) {
      ...
      setShoppingCart((prevState) => [...prevState, updatedShoppingCartItem[0]])
